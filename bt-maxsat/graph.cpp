...
Graph::Graph(int n)
  : n_(n), m_(0), adj_list_(n), adj_mat_(n, n), adj_mat2_(n) {
...
void Graph::AddEdge(int v, int u) {
  ...
  adj_mat2_[v].Set(u, true);
  adj_mat2_[u].Set(v, true);
}
...
void Graph::RemoveEdge(int v, int u) {
  ...
  adj_mat_[v][u] = false;
  adj_mat_[u][v] = false;
  adj_mat2_[v].Set(u, false);
  adj_mat2_[u].Set(v, false);
  int fo = 0;
...
void Graph::Dfs2(int v, Bitset& sep, Bitset& vis, std::vector<int>& f) const {
  ...
  while (i < s) {
    ...
    for (int j = 0; j < chunks; j++) {
      ...
      while (go) {
        vis.data_[j] &= (~(go&-go));
        q[s++] = __builtin_ctzll(go) + j*64;
        go &= ~-go;
      }
    }
    for (int j = 0; j < chunks; j++) {
      uint64_t go = sep.data_[j] & adj_mat2_[x].data_[j];
      while (go) {
        sep.data_[j] &= (~(go&-go));
        f.push_back(__builtin_ctzll(go) + j*64);
        go &= ~-go;
      }
    }
  }
